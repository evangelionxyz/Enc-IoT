...
void aes_encrypt_func(const char *padded_data, unsigned char *out, const size_t padded_size, AES_KEY key, size_t iterations)
{
    if (iterations < 1)
        return;
    for (size_t i = 0; i < padded_size; i += AES_BLOCK_SIZE)
        AES_encrypt((const unsigned char *)padded_data + i, out + i, &key);
    aes_encrypt_func(padded_data, out, padded_size, key, iterations - 1);
}

void aes_decrypt_func(const unsigned char* encrypted_data, unsigned char *out, const size_t padded_size, AES_KEY key, size_t iterations)
{
    if (iterations < 1)
        return;
    for (size_t i = 0; i < padded_size; i += AES_BLOCK_SIZE)
        AES_decrypt((const unsigned char *)encrypted_data + i, out + i, &key);
    aes_decrypt_func(encrypted_data, out, padded_size, key, iterations - 1);
}

void aes_recursive_test(const std::string &data, std::stringstream &logging_stream, size_t iterations)
{
    logging_stream << iterations << " Iterations Recursive\n";
    timer scoped_timer;
    ...
    {
        timer t;
        aes_encrypt_func(padded_data.c_str(), encrypted_data.data(),
            padded_size, aes_encryption_key, iterations);
        float elapsed = t.elapsed_ms();
        logging_stream << "Encryption elapsed time: " << elapsed << " ms, " << elapsed * 0.001f << " s\n";
    }
    ...
    {
        timer decryption_timer;
        aes_decrypt_func(encrypted_data.data(), decrypted_data.data(),
            padded_size, aes_decryption_key, iterations);
        float elapsed = decryption_timer.elapsed_ms();
        logging_stream << "Decryption elapsed time: " << elapsed << " ms, " << elapsed * 0.001f << " s\n";
    }
...
void rsa_encrypt_func(const char *data, size_t data_size, unsigned char *out, RSA *key_pair, size_t chunk_size, size_t total_chunks, size_t iterations)
{
    if (iterations < 1)
        return;
    ...
    {
        size_t chunk_start = chunk * chunk_size;
        size_t chunk_length = std::min(chunk_size, data_size - chunk_start);
        int encrypted_size = RSA_public_encrypt(chunk_length,
            (const unsigned char *)data + chunk_start,
            out + offset, key_pair, RSA_PKCS1_OAEP_PADDING);
        if (encrypted_size == -1)
        {
            ERR_print_errors_fp(stderr);
            printf("Encrypt error\n");
    ...
    rsa_encrypt_func(data, data_size, out, key_pair, chunk_size, total_chunks, iterations - 1);
}

void rsa_decrypt_func(unsigned char *encrypted_data, unsigned char *out, RSA *key_pair, size_t chunk_size, size_t total_chunks, size_t iterations)
{
    if (iterations < 1)
        return;
    ...
    for (size_t chunk = 0; chunk < total_chunks; ++chunk)
    {
        int decrypted_size = RSA_private_decrypt(RSA_size(key_pair),
            encrypted_data + offset, out + chunk * chunk_size,
            key_pair, RSA_PKCS1_OAEP_PADDING);
        ...
        offset += RSA_size(key_pair);
    }

    rsa_decrypt_func(encrypted_data, out, key_pair, chunk_size, total_chunks, iterations - 1);
}

void rsa_recursive_test(const std::string &data, std::stringstream &logging_stream, size_t iterations)
{
    logging_stream << iterations << " Iterations\n";
    timer scoped_timer;
    RSA *rsa_keypair = RSA_new();
    ...
    {
        timer t;
        rsa_encrypt_func(data.c_str(), data.size(), encrypted_data.data(),
            rsa_keypair, chunk_size, total_chunks, iterations);
        float elapsed = t.elapsed_ms();
        logging_stream << "Encryption elapsed time: " << elapsed << " ms, " << elapsed * 0.001f << " s\n";
    }
    ...
    {
        timer decryption_timer;
        rsa_decrypt_func(encrypted_data.data(), decrypted_data.data(), rsa_keypair,
            chunk_size, total_chunks, iterations);
        float elapsed = decryption_timer.elapsed_ms();
        logging_stream << "Decryption elapsed time: " << elapsed << " ms, " << elapsed * 0.001f << " s\n";
    }
...
void ecc_encrypt_func(VecUChar &encrypted_data, const char *data, size_t data_size, const unsigned char *aes_key, const unsigned char *iv, size_t iterations)
{
    if (iterations < 1)
        return;
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    ...
    ecc_encrypt_func(encrypted_data, data, data_size, aes_key, iv, iterations - 1);
}

void ecc_decrypt_func(VecUChar &decrypted_data, VecUChar &encrypted_data, const unsigned char *aes_key, const unsigned char *iv, size_t iterations)
{
    if (iterations < 1)
        return;
    ...
    ecc_decrypt_func(decrypted_data, decrypted_data, aes_key, iv, iterations - 1);
}

void ecc_recursive_test(const std::string &data, std::stringstream &logging_stream, size_t iterations)
{
    logging_stream << iterations << " Iterations\n";
    timer scoped_timer;
    ...
    {
        timer t;
        ecc_encrypt_func(encrypted_data, data.c_str(), data.size(), aes_key, iv, iterations);
        float elapsed = t.elapsed_ms();
        logging_stream << "Encryption elapsed time: " << elapsed << " ms, " << elapsed * 0.001f << " s\n";
    }
    ...
    {
        timer decryption_timer;
        ecc_decrypt_func(decrypted_data, encrypted_data, aes_key, iv, iterations);
        float elapsed = decryption_timer.elapsed_ms();
        logging_stream << "Decryption elapsed time: " << elapsed << " ms, " << elapsed * 0.001f << " s\n";
    }
...
int main(int argc, char **argv) {
    ...
    aes_recursive_test(data, logging_stream, 1000);

    logging_stream << "RSA Encryption Benchmark " << filename << '\n';
    rsa_recursive_test(data, logging_stream, 100);
    rsa_recursive_test(data, logging_stream, 500);
    rsa_recursive_test(data, logging_stream, 1000);
